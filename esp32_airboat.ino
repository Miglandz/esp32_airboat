...
float batteryCapacity = BATTERY_CAPACITY; // From config
int16_t altitude = 0;
...
void setup() {
  ...
  // Initialize current sensor (ACS758)
  setupCurrentSensor();
  ...
  Serial.println("ESP32 Airboat Controller with GPS RTH Ready!");
}
...
void loop() {
  ...
  // Update battery data from current sensor
  updateBatteryData();
...
void setupCurrentSensor() {
  // Initialize ADC for ACS758 current sensor and voltage sensor
  pinMode(CURRENT_SENSOR_PIN, INPUT);
  pinMode(VOLTAGE_SENSOR_PIN, INPUT);
  analogReadResolution(12); // 12-bit resolution for ESP32
  Serial.println("Current and voltage sensors initialized");
}
...
void setupMPU6050() {
  ...
  if (!mpu.begin()) {
    ...
    // Set up the accelerometer with stronger filtering for airboat vibrations
    mpu.setAccelerometerRange(MPU6050_RANGE_8_G);
    mpu.setGyroRange(MPU6050_RANGE_500_DEG);
    mpu.setFilterBandwidth(MPU6050_BAND_5_HZ); // Stronger filtering for airboat vibrations
  }
}
...
void extractChannels(uint8_t *frame) {
  ...
  Serial.print(" V: "); Serial.print(batteryVoltage, 1);
  Serial.print("V I: "); Serial.print(batteryCurrent, 1);
  Serial.println("A");
}
...
void runLedEffect() {
  ...
  // Battery warning
  if (batteryVoltage < 3.5 * BATTERY_CELLS) {
    // Low battery warning - flash third LED red
    if ((millis() / 250) % 2 == 0) {
      leds[2] = CRGB::Red;
    }
  }
...
void processAccelerometer() {
  ...
  // Apply a simple low-pass filter to reduce vibrations for airboat
  static float accelXFiltered = 0, accelYFiltered = 0, accelZFiltered = 0;
  accelXFiltered = accelXFiltered * 0.8 + a.acceleration.x * 0.2;
  accelYFiltered = accelYFiltered * 0.8 + a.acceleration.y * 0.2;
  accelZFiltered = accelZFiltered * 0.8 + a.acceleration.z * 0.2;
  
  // Save raw values
  accelX = accelXFiltered;
  accelY = accelYFiltered;
  accelZ = accelZFiltered;
  gyroX = g.gyro.x;
  gyroY = g.gyro.y;
  gyroZ = g.gyro.z;
...
void updateBatteryData() {
  ...
  // Read current using ACS758 sensor
  int currentRaw = analogRead(CURRENT_SENSOR_PIN);
  float voltage_current = (currentRaw / 4095.0) * 3.3;
  batteryCurrent = (voltage_current - ACS758_OFFSET) / ACS758_SENSITIVITY;
  
  // Ensure current is not negative (sensor offset calibration)
  if (batteryCurrent < 0) batteryCurrent = 0;
...
void sendBatteryTelemetry() {
  ...
  uint8_t fuelPercent = constrain((batteryCapacity / BATTERY_CAPACITY) * 100, 0, 100);
  frame[7] = fuelPercent;
